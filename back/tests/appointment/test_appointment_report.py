...
def prepare_days(base_date: datetime):
    yesterday = AppointmentFactory.create_batch(3)
    before_yesterday = AppointmentFactory.create_batch(2)
    three_days_ago = AppointmentFactory.create_batch(1)

    for x in yesterday:
        x.datetime = base_date - timedelta(days=1)
        x.save()
        
    for x in before_yesterday:
        x.datetime = base_date - timedelta(days=2)
        x.save()

    for x in three_days_ago:
        x.datetime = base_date - timedelta(days=3)
        x.save()


@pytest.mark.django_db
def test_appointment_report(client: APIClient):
    now = datetime.now()
    prepare_days(now)
